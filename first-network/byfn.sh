...
function upgradeNetwork () {
  docker inspect  -f '{{.Config.Volumes}}' orderer.legitcheck.com |grep -q '/var/hyperledger/production/orderer'
  if [ $? -ne 0 ]; then
  ...
  docker-compose $COMPOSE_FILES stop orderer.legitcheck.com
  docker cp -a orderer.legitcheck.com:/var/hyperledger/production/orderer $LEDGERS_BACKUP/orderer.legitcheck.com
  docker-compose $COMPOSE_FILES up -d --no-deps orderer.legitcheck.com

  for PEER in peer0.atos.legitcheck.com peer1.atos.legitcheck.com peer0.esme.legitcheck.com peer1.esme.legitcheck.com; do
...
function networkDown () {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    # remove the docker-compose yaml file that was customized to the legitcheck
...
function replacePrivateKey () {
  ...
  CURRENT_DIR=$PWD
  cd crypto-config/peerOrganizations/atos.legitcheck.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
  sed $OPTS "s/CA1_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yaml
  cd crypto-config/peerOrganizations/esme.legitcheck.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
...
# and two Peer Orgs (``Atos`` & ``ESME``) each managing and maintaining two peer nodes.
...
# (``peer0.atos.legitcheck.com`` & ``peer0.esme.legitcheck.com``).  Secondly, we point to
...
function generateChannelArtifacts() {
  ...
  echo
  echo "#################################################################"
  echo "#######    Generating anchor peer update for AtosMSP   ##########"
  echo "#################################################################"
  set -x
  configtxgen -profile TwoOrgsChannel -outputAnchorPeersUpdate ./channel-artifacts/AtosMSPanchors.tx -channelID $CHANNEL_NAME -asOrg AtosMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to generate anchor peer update for AtosMSP..."
    exit 1
  fi

  echo
  echo "#################################################################"
  echo "#######    Generating anchor peer update for ESMEMSP   ##########"
  echo "#################################################################"
  set -x
  configtxgen -profile TwoOrgsChannel -outputAnchorPeersUpdate \
  ./channel-artifacts/ESMEMSPanchors.tx -channelID $CHANNEL_NAME -asOrg ESMEMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to generate anchor peer update for ESMEMSP..."
    exit 1
  fi
  echo
}
